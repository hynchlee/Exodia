	...
	<%@ include file="/WEB-INF/views/common/header.jsp" %>
		...
		<main>
            ...
            <c:forEach items="${tvolist}" var="tvo">
	            <select name="lectureList" class="choiceLecture">
	                <option value="${tvo.lectureNo}">${tvo.lectureCategoryName}</option>
	            </select>
            </c:forEach>
...
<script>
   ...
   function tclassList(){
        ...
        const identity = '${loginMember.identity}';

        if(identity =='S'){
            $.ajax({
                url: '${root}/class/tlist',
                type: "GET",
                data: {
                    tvolistNo : '${tvolist.lectureNo}'
                }
            });
        }
