...
//	}
	...
	//우리반 페이징
	public int getBoardClassListCnt(Connection conn, String lno) throws Exception {
...
//		}
	...
	public List<BoardVo> getBoardFreeList(Connection conn, PageVo pv) throws Exception {
		...
		String sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT B.BOARD_NO, B.BOARD_CATEGORY_NO, B.MEMBER_NO, B.BOARD_TITLE, B.BOARD_CONTENT, TO_CHAR(B.ENROLL_DATE, 'YYYY.MM.DD') AS ENROLL_DATE, TO_CHAR(B.MODIFY_DATE, 'YYYY.MM.DD') AS MODIFY_DATE, B.STATUS, B.HIT, M.MEMBER_NICK, C.BOARD_CATEGORY_TYPE, COUNT(CASE WHEN R.STATUS = 'O' THEN R.REPLY_NO END) AS TOTAL_REPLIES FROM BOARD B JOIN MEMBER M ON (B.MEMBER_NO = M.MEMBER_NO) JOIN BOARD_CATEGORY C ON (B.BOARD_CATEGORY_NO = C.BOARD_CATEGORY_NO) LEFT JOIN REPLY R ON (B.BOARD_NO = R.BOARD_NO AND R.STATUS = 'O') WHERE B.STATUS = 'O' AND C.BOARD_CATEGORY_NO = 1 GROUP BY B.BOARD_NO, B.BOARD_CATEGORY_NO, B.MEMBER_NO, B.BOARD_TITLE, B.BOARD_CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.STATUS, B.HIT, M.MEMBER_NICK, C.BOARD_CATEGORY_TYPE ORDER BY B.BOARD_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, pv.getBeginRow());
		pstmt.setInt(2, pv.getLastRow());
		ResultSet rs = pstmt.executeQuery();
	...
	//자유 검색해서 목록 조회
		public List<BoardVo> getBoardFreeList(Connection conn, PageVo pv, String searchType, String searchValue) throws Exception {
			...
			if (searchType.equals("freeTitle")) {
				//제목 검색
				sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT B.BOARD_NO, B.BOARD_CATEGORY_NO, B.MEMBER_NO, B.BOARD_TITLE, B.BOARD_CONTENT, TO_CHAR(B.ENROLL_DATE, 'YYYY.MM.DD') AS ENROLL_DATE, TO_CHAR(B.MODIFY_DATE, 'YYYY.MM.DD') AS MODIFY_DATE, B.STATUS, B.HIT, M.MEMBER_NICK, C.BOARD_CATEGORY_TYPE, COUNT(CASE WHEN R.STATUS = 'O' THEN R.REPLY_NO END) AS TOTAL_REPLIES FROM BOARD B JOIN MEMBER M ON (B.MEMBER_NO = M.MEMBER_NO) JOIN BOARD_CATEGORY C ON (B.BOARD_CATEGORY_NO = C.BOARD_CATEGORY_NO) LEFT JOIN REPLY R ON (B.BOARD_NO = R.BOARD_NO AND R.STATUS = 'O') WHERE B.STATUS = 'O' AND C.BOARD_CATEGORY_NO = 1 AND B.BOARD_TITLE LIKE ('%' || ? || '%') GROUP BY B.BOARD_NO, B.BOARD_CATEGORY_NO, B.MEMBER_NO, B.BOARD_TITLE, B.BOARD_CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.STATUS, B.HIT, M.MEMBER_NICK, C.BOARD_CATEGORY_TYPE ORDER BY B.BOARD_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
			}else if (searchType.equals("freeContent")) {
				//내용 검색
				sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT B.BOARD_NO, B.BOARD_CATEGORY_NO, B.MEMBER_NO, B.BOARD_TITLE, B.BOARD_CONTENT, TO_CHAR(B.ENROLL_DATE, 'YYYY.MM.DD') AS ENROLL_DATE, TO_CHAR(B.MODIFY_DATE, 'YYYY.MM.DD') AS MODIFY_DATE, B.STATUS, B.HIT, M.MEMBER_NICK, C.BOARD_CATEGORY_TYPE, COUNT(CASE WHEN R.STATUS = 'O' THEN R.REPLY_NO END) AS TOTAL_REPLIES FROM BOARD B JOIN MEMBER M ON (B.MEMBER_NO = M.MEMBER_NO) JOIN BOARD_CATEGORY C ON (B.BOARD_CATEGORY_NO = C.BOARD_CATEGORY_NO) LEFT JOIN REPLY R ON (B.BOARD_NO = R.BOARD_NO AND R.STATUS = 'O') WHERE B.STATUS = 'O' AND C.BOARD_CATEGORY_NO = 1 AND B.BOARD_CONTENT LIKE ('%' || ? || '%') GROUP BY B.BOARD_NO, B.BOARD_CATEGORY_NO, B.MEMBER_NO, B.BOARD_TITLE, B.BOARD_CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.STATUS, B.HIT, M.MEMBER_NICK, C.BOARD_CATEGORY_TYPE ORDER BY B.BOARD_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
			}else {
		...
		public List<ReplyVo> getBoardReplyList(Connection conn, String bno) throws Exception {
			...
			String sql = "SELECT R.REPLY_NO, R.WRITER_NO, R.BOARD_NO, R.REPLY_CONTENT, TO_CHAR(R.ENROLL_DATE, 'YYYY-MM-DD HH24:MI:SS') AS ENROLL_DATE, TO_CHAR(R.MODIFY_DATE, 'YYYY-MM-DD HH24:MI:SS') AS MODIFY_DATE, R.STATUS, M.MEMBER_NICK AS WRITER_NICK, ( SELECT COUNT(A.ANSWER_NO) FROM ANSWER A WHERE A.REPLY_NO2 = R.REPLY_NO AND A.STATUS = 'O' ) AS ANSWER_COUNT FROM REPLY R JOIN MEMBER M ON (M.MEMBER_NO = R.WRITER_NO) WHERE R.STATUS = 'O' AND R.BOARD_NO = ? GROUP BY R.REPLY_NO, R.WRITER_NO, R.BOARD_NO, R.REPLY_CONTENT, R.ENROLL_DATE, R.MODIFY_DATE, R.STATUS, M.MEMBER_NICK ORDER BY R.REPLY_NO DESC";
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, bno);
			ResultSet rs = pstmt.executeQuery();
			...
			while (rs.next()) {
				...
				String answerCount = rs.getString("ANSWER_COUNT");
...
//		}
		...
		public List<BoardVo> getBoardClassList(Connection conn, PageVo pv, String studentMemberNo) throws SQLException {
			...
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, studentMemberNo);
			pstmt.setInt(2, pv.getBeginRow());
			pstmt.setInt(3, pv.getLastRow());
			ResultSet rs = pstmt.executeQuery();
		...
		public int getBoardSClassListCnt(Connection conn, String studentMemberNo) throws Exception {
...
//			}
		...
		//학생 번호 가져오기
		public String getStudentMemberNo(Connection conn, String memberNo) throws Exception {
			
			 String sql = "SELECT S.STUDENT_MEMBER_NO FROM STUDENT S JOIN MEMBER M ON S.STUDENT_MEMBER_NO = M.MEMBER_NO WHERE M.MEMBER_NO = ?";
			 PreparedStatement pstmt = conn.prepareStatement(sql);
             pstmt.setString(1, memberNo);
             ResultSet rs = pstmt.executeQuery();
             
             String studentMemberNo = null;
             if (rs.next()) {
                 studentMemberNo = rs.getString("STUDENT_MEMBER_NO");
             }
		...
		public int replyDelete(Connection conn, String rno) throws Exception {
			
			String sql = "UPDATE REPLY SET STATUS='X' WHERE REPLY_NO=?";
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, rno);
			int result = pstmt.executeUpdate();
